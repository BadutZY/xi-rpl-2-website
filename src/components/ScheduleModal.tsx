          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold font-heading gradient-text">
                Detail Jadwal {dayLabel}
              </h3>
              <button onClick={onClose} aria-label="Tutup" className="text-muted-foreground hover:text-foreground cursor-pointer">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              {details.map((item, idx) => (
                <div key={idx} className="modal-lesson-item">
                  <div className="mb-3">
                    <span className="subject-badge px-3 py-1 rounded-full text-sm">{item.subject}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <GraduationCap size={20} className="text-secondary flex-shrink-0" />
                    <span className="text-foreground font-medium text-sm">{item.teacher}</span>
                  </div>
                </div>
              ))}
            </div>
